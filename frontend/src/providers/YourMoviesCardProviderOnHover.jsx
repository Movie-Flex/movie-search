...
const CardProviderOnHover = ({
    ...
    const handleRatingSubmission = async () => {
       try{
        const response =await axios.post(`http://localhost:3002/api/rateMovie/${movie._id}/${userRating * 2}`, {}, {
            headers: {
    ...
    const handleDeleteFromFavourites = async () => {
       try{
        const response=await axios.post(`http://localhost:3002/api/deleteFavouriteMovie/${movie._id}`, {}, {
        headers: {
            'authorization': `Bearer ${token}`
        }
    })
    ...
    const handleDeleteFromWatchLater =async () => {
       ...
       try{
        const response=await axios.post(`http://localhost:3002/api/deleteWatchLaterMovie/${movie._id}`, {}, {
            headers: {
                'authorization': `Bearer ${token}`
            }
        })
            ...
            <Card width='300px' height='400px' align='center' bg='none' variant='elevated' borderRadius='0px'>
                <CardBody>
                    ...
                    <Stack marginTop='10px' display='flex' flexDirection='row' gap='5px' alignItems='center'>
                        ...
                        <Button onClick={() => {handleDeleteFromWatchLater()}} variant='solid' textColor='#171D21' colorScheme='yellow'>
                            Delete from Watch Later
                        </Button>
                    </Stack>
                    <Stack marginTop='10px' display='flex' flexDirection='row' gap='5px' alignItems='center'>
                        <FaRegStar className='text-[#ECC94B] text-xl mx-1' />
                        <Button onClick={() => { handleDeleteFromFavourites() }} variant='solid' textColor='#171D21' colorScheme='yellow'>
