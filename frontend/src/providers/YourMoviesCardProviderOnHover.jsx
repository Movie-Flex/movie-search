...
const CardProviderOnHover = ({
  ...
  const handleRatingSubmission = async () => {
    try {
      const response = await axios.post(
        `http://localhost:3002/api/rateMovie/${movie._id}/${userRating * 2}`,
        {},
        {
          headers: {
  ...
  const handleDeleteFromFavourites = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:3002/api/deleteFavouriteMovie/${movie._id}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
  ...
  const handleDeleteFromWatchLater = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:3002/api/deleteWatchLaterMovie/${movie._id}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
